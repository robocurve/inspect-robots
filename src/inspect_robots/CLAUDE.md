...
| `_video.py` | Shared strict-uint8 ffmpeg core with stderr temp files and per-stream failure isolation; `inspect-robots video` retains hard launch-failure behavior and filesystem MP4 output, while the report-only wrapper returns temporary MP4 bytes and degrades every failure without leaking files |
