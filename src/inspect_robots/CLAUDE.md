...
| `_html.py` | `render_html()`: a saved `EvalLog` as one HTML page, with synchronized multicamera players, live transcript step rails, exact-match stored-frame correlation, and a shared payload budget (everything escaped once at interpolation, bounded JSON fallback); owns the shared chat-transcript predicates and status display map that `cli.py` imports |
| `_html_index.py` | `render_index()`: directory report index with presentation run numbers, persisted filtering and column sorting, id/stamp search, and guarded row-click navigation |
...
| `_video.py` | Discover per-trial MP4 sidecars for HTML reports and implement `inspect-robots video`: reunite a log with its `FrameStore` side-cars and pipe them to ffmpeg as faststart MP4s, one per (trial, camera) stream (stderr temp file not pipe, per-stream failure isolation, strict uint8) |
