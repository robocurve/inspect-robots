...
| `_html.py` | `render_html()`: a saved or running `EvalLog` as one self-contained HTML page, with live refresh/banner and pending-score rendering, exact-match stored-frame correlation, newest-first live allocation through a shared encode cache and effective payload budget, and started-page wire-media elision (everything escaped once at interpolation, bounded JSON fallback); owns the shared chat-transcript predicates and status display map that `cli.py` imports |
