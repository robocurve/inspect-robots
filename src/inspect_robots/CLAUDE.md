...
| `eval.py` | `eval()` / `eval_set()` orchestration, including per-run `bind_spaces` and `bind_frames_dir` sink extension binding plus source-preserving operator-message persistence with console fallback for old events; `grader=` (object or registry name) and `before_scoring=` are the same pre-scoring seam, mutually exclusive, validated against the `Grader` protocol |
