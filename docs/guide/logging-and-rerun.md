...
Every run produces an immutable [`EvalLog`][roboinspect.log.EvalLog] — the canonical,
...
```python
from roboinspect import eval, read_eval_log
...
a read-back guarantee: a newer RoboInspect always reads an older log.

## Sinks

A [`LogSink`][roboinspect.logging.LogSink] observes the run lifecycle
(`on_eval_start` → per trial `on_trial_start`/`log_step`/`on_trial_end` →
`on_eval_end`). Builtins:

- [`JsonLogSink`][roboinspect.logging.JsonLogSink] — always on; the canonical JSON record.
- [`RerunSink`][roboinspect.logging.RerunSink] — optional, lazily imported.
...
no-ops, so core never depends on it. Install with `pip install "roboinspect[rerun]"`.

## Frame side-cars

Camera frames are large. With `store_frames=True`, the rollout streams frames to
`<log_dir>/frames` through a [`FrameStore`][roboinspect.frames.FrameStore] and the
`TrialRecord` keeps lightweight [`FrameRef`][roboinspect.frames.FrameRef] handles — so
