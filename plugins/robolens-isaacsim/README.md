...
plugin for [RoboInspect](https://github.com/robocurve/roboinspect) — the "Inspect AI
for robotics".

RoboInspect factors a robotics eval into two swappable inputs: a `Policy` (the VLA
...
pip install roboinspect robolens-isaacsim
```

## Use it

The embodiment is discovered through the `roboinspect.embodiments` entry point, so
...
RoboInspect fail-fast-checks the `(policy, embodiment)` pair before any rollout. To
...
| `image_keys` / `state_keys` | map RoboInspect keys → your task's raw dict keys |
...
- **Stream frames to disk for long episodes.** RoboInspect keeps each step's
