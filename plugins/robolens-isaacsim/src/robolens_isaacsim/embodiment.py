...
This is the "body + world" half of a RoboInspect eval, backed by a real physics
simulation. It conforms to :class:`roboinspect.Embodiment` (the runtime-checkable
protocol), so once installed it can be paired with any compatible
:class:`roboinspect.Policy` and run through ``roboinspect.eval``.

Design mirrors RoboInspect's own ``RerunSink``: the heavy, GPU-bound, non-PyPI
...
on any machine (so ``roboinspect list embodiments`` and the fail-fast compatibility
...
from roboinspect.spaces import CANONICAL_STATE_UNITS

if TYPE_CHECKING:
    from roboinspect import Action
...
# GPU allows; RoboInspect owns wall-clock pacing).
...
def _default_state_fields(num_arm_joints: int) -> tuple[StateField, ...]:
    """Canonical proprioception for a Franka-like arm (keys/units from RoboInspect)."""
...
class IsaacSimEmbodiment:
    """Wrap an Isaac Lab gym task as a RoboInspect :class:`~roboinspect.Embodiment`.
    ...
    cameras:
        ...
        tuples. Match the names your policy requires (RoboInspect remaps if needed).
    headless / device:
    ...
    def __init__(
        ...
        *,
        ...
        # Map RoboInspect obs keys -> the task's raw dict keys. Identity by default.
    ...
    # ------------------------------------------------------------------ #
    # Translation: Isaac Lab dict/tensors <-> RoboInspect types
    # ------------------------------------------------------------------ #
